...
- restore `--combine-strands` for the haplotype-level and population-scale `modkit pileup` when `--all_contexts` is not set (CG is palindromic, so DSS sees one locus per CpG with the coverage of both strands, as in v2.0.0); it stays off under `--all_contexts` and under `--m6a`, where modkit rejects it (multiple motifs, and A-base modifications with a CpG motif, respectively; #77)
